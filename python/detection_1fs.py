...
def detect_objects(video_path, uploader_id):
    ...
    temp_avi_path = os.path.join(videos_dir, f"{upload_id}_temp_1s.avi")
    final_mp4_path = os.path.join(videos_dir, f"{upload_id}_detected_1s.mp4")
    ...
    out = cv2.VideoWriter(temp_avi_path, fourcc, 1.0, (width, height))  # 1 FPS output
    ...
    while True:
        ...
        # Process only 1 frame every second
        ...
        for box in results.boxes:
            ...
            # === Save key frame ===
            frame_filename = f"{upload_id}_1s_{processed_frames}_{label}.jpg"
    ...
    meta_file = os.path.join(OUTPUT_DIR, f"{upload_id}_1s_meta.json")
